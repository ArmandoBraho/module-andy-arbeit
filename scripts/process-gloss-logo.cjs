...
const SOURCE = String.raw`C:\Users\arman\.cursor\projects\c-armando-develop-AndyArbeit\assets\logo-andyarbeit-gloss.png`;
const ROOT = path.join(__dirname, '..');
const DEST = path.join(ROOT, 'public', 'logo-andyarbeit-gloss.png');
const COLOR_TOL = 28;
const NEAR_BLACK_MAX = 30;
...
function isOrange(r, g, b) {
  // Strong red chroma — preserve brand orange
...
function isChrome(r, g, b) {
  ...
  // Silver/chrome bevel: mid-to-high L, some channel spread, not orange
...
function isNearWhite(r, g, b) {
  return r >= 255 - COLOR_TOL && g >= 255 - COLOR_TOL && b >= 255 - COLOR_TOL;
}

function isFlatGrey(r, g, b) {
  const spread = chroma(r, g, b);
  const L = luminance(r, g, b);
  return spread <= COLOR_TOL && L > NEAR_BLACK_MAX && L < 255 - COLOR_TOL;
}

function isFlatDark(r, g, b) {
  return chroma(r, g, b) <= COLOR_TOL && luminance(r, g, b) <= 60;
}
...
/** Flat background plates only (not chrome/orange). */
function isBackgroundPlate(r, g, b) {
  if (isLogoInk(r, g, b)) return false;
  return isNearBlack(r, g, b) || isNearWhite(r, g, b) || isFlatGrey(r, g, b);
}
...
/** Enclosed near-black / flat-dark regions in upper part (letter holes). */
function punchEnclosedHoles(png) {
  ...
  function expandable(r, g, b) {
    if (isLogoInk(r, g, b)) return false;
    return isNearBlack(r, g, b) || isFlatDark(r, g, b);
  }
...
function processLogo(png) {
  ...
  const outerPunched = floodFromSeeds(png, borderSeeds, (_x, _y, r, g, b) =>
    isBackgroundPlate(r, g, b)
  );
...
function main() {
  ...
  if (fs.existsSync(distDir)) {
    const distDest = path.join(distDir, 'logo-andyarbeit-gloss.png');
    fs.writeFileSync(distDest, outBuf);
    console.log('Wrote:', distDest, '(' + outBuf.length + ' bytes)');
  } else {
    console.log('dist/ not present — skipped dist copy');
  }

  // Confirm we did not touch other logos
  for (const name of ['logo.png', 'logo-transparent.png', 'logo-from-design.png']) {
    const p = path.join(ROOT, 'public', name);
    console.log('Untouched check exists:', name, fs.existsSync(p));
  }
